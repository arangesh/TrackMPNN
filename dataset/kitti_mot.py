...
def store_results_kitti(bbox_pred, y_out, output_path):
    ...
    bbox_pred [num_dets, (x1, y1, x2, y2, score)]: Predicted bboxes in a sequence
...
class KittiMOTDataset(data.Dataset):
    ...
    def load_kitti_labels(self, fr):
        ...
        """
        annotations = []
        bbox_gt = np.zeros((0, 15), dtype=np.float32)
        if self.label_path is None:
            return annotations, bbox_gt

        label_file = open(os.path.join(self.label_path, input_info[0] + '.txt'), 'r')
        for line in label_file:
            ...
            # [fr, trk_id, alpha, x1, y1, x2, y2, h, w, l, x, y, z, rotation_y, score]
            b = [ann['frame'], ann['track_id']] + [ann['alpha']] + ann['bbox'] + ann['dim'] \
                + ann['location'] + [ann['rotation_y']] + [1]
            bbox_gt = np.concatenate((bbox_gt, np.array([b], dtype=np.float32)), axis=0)
        return annotations, bbox_gt

    def get_trk_feats(self, feat_maps, bboxes, im_sizes):
        """
        ...
        for i, bbox in enumerate(bboxes):
            ...
            c_x = (c_x * self.detector.opt.input_w) / im_sizes[i][0]
            c_y = (c_y * self.detector.opt.input_h) / im_sizes[i][1]
    ...
    def get_track_ids(self, bbox_pred, bbox_gt, iou_thresh=0.5):
        ...
        bbox_pred: [N_pred, (fr, -1, alpha, x1, y1, x2, y2, h, w, l, x, y, z, rotation_y, score)]
        bbox_gt: (N_gt, (fr, trk_id, alpha, x1, y1, x2, y2, h, w, l, x, y, z, rotation_y, score)]
        """
        if bbox_pred.size == 0 or bbox_gt.size == 0:
            return bbox_pred

        if not np.all(bbox_pred[:, 0] == bbox_gt[:, 0]):
            assert False, "Detections and GT boxes not from same frame!"

        # calculate iou matrix
        ious = vectorized_iou(bbox_pred[:, 3:7], bbox_gt[:, 3:7])
        # find detections with max overlaps with GT
        ids_max_iou = np.argmax(ious, axis=0).tolist()

        for col, row in enumerate(ids_max_iou):
            if iou[row, col] >= iou_thresh:
    ...
    def __getitem__(self, index):
        ...
        bbox_pred = np.zeros((0, 15), dtype=np.float32)
        bbox_gt = np.zeros((0, 15), dtype=np.float32)
        im_sizes = []
        ...
        for fr in range(input_info[1], input_info[2], 1):
            ...
            im_sizes.append((im.shape[1], im.shape[0]))
            
            # load GT annotations
            # [fr, trk_id, alpha, x1, y1, x2, y2, h, w, l, x, y, z, rotation_y, score]
            annotations, bbox_gt_fr = self.load_kitti_labels(fr)
            ...
            if random_transforms_hf:
                ...
                bbox_gt_fr[:, 2] = -bbox_gt_fr_old[:, 2] # alpha
                bbox_gt_fr[:, 3] = im_sizes[-1][0] - bbox_gt_fr[:, 5] # x1
                bbox_gt_fr[:, 5] = im_sizes[-1][0] - bbox_gt_fr[:, 3] # x2
                bbox_gt_fr[:, 10] = -bbox_gt_fr_old[:, 10] # x
                bbox_gt_fr[:, 13] = np.pi - bbox_gt_fr_old[:, 13] # rotation_y
            ...
            detections = detector_ops['results'][2]
            # find bboxes for the frame
            # [fr, -1, alpha, x1, y1, x2, y2, h, w, l, x, y, z, rotation_y, score]
            bbox_pred_fr = np.zeros((detections.shape[0], 15), dtype=np.float32)
            bbox_pred_fr[:, 0] = fr
            bbox_pred_fr[:, 1] = -1
            bbox_pred_fr[:, 2:] = detections
        ...
        # [score, x1, y1, x2, y2]
        two_d_feats = self.convert_to_tensor(bbox_pred[:, [14, 3, 4, 5, 6]])
        # [h, w, l, X, Y, Z, rotation_y]
        three_d_feats = self.convert_to_tensor(bbox_pred[:, [7, 8, 9, 10, 11, 12, 13]])
        im_feats = self.get_trk_feats(detector_ops['outputs']['trk'], bbox_pred[:, [3, 4, 5, 6]], im_sizes)
